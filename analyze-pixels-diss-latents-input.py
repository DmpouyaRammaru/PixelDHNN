...
from data_get_latents import get_dataset, make_gym_dataset, sample_gym, hamiltonian_fn
...
plt.plot(coords[:k,0], coords[:k,1], linestyle='-', marker=None, color = 'red')
...
def Hplot(model,name='',save=False,dir_name=''):
    ...
    ax1.set_title('True Hamiltonian', pad=10)
    ax1.set_xlim(-R, R)
    ax1.set_ylim(-R, R)
...
Hplot(hnn_model,name='HNN',save=False,dir_name='')
# %% [markdown]
...
base_y0 = AE_model.encode(x0).detach().numpy().squeeze()
hnn_y0 = AE_model.encode(x0).detach().numpy().squeeze()
...
hnn_pixel_traj = AE_model.decode(hnn_zs).detach().numpy()
...
#[:tmax]→[tmin:tmax]に変更
plt.plot(t_eval[tmin:tmax], hnn_cc[tmin:tmax,0], 'b-', label='DHNN', linewidth=3)
...
plt.plot(hnn_cc[tmin:,0], hnn_cc[tmin:,1], 'b-', label='Hamiltonian NN')
