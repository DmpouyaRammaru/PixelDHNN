...
base_model = load_model(args, baseline=False)
...
# get scalar fields
F2 = hnn_model.hnn(mesh_inputs)
F1 = hnn_model.diss(mesh_inputs)
print(f'F1.shape:{F1.shape}') #F1.shape:torch.Size([2500, 1])
...
F1 = hnn_model.hnn(z_values)
F2 = hnn_model.diss(z_values)
energy = F1.detach().numpy() + F1.detach().numpy()
plt.plot(energy)
