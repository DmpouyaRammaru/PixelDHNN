...
def pixelhnn_loss(x, x_next, model, return_scalar=True):
  ...
  # 入力z_nextと予測値z_hat_nextが必要．これでやると何故か最もうまくいく( z_hat_next = 2 * z + model.time_derivative(z) ,hnn_loss = ((2 * z_next - z_hat_next)**2).mean(1))
  z_hat_next = z + model.time_derivative(z) 
  hnn_loss = (( z_next - z_hat_next)**2).mean(1)
  # dtはなくて良い
  # hnnloss がdt倍大きい
  # dotx = (z_next - z) / dt
  # dotx_hat = model.time_derivative(z) 
  # hnn_loss = ((dotx_hat - dotx)**2).mean(1)
  ...
  # loss = ae_loss + cc_loss + 1e-1 * dt *  hnn_loss
